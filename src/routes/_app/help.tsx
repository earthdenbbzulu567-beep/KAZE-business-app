...
import { dismissTutorial } from "@/lib/kaze/server";
...
function HelpPage() {
  return (
    <div className="kaze-enter space-y-8">
      <PageHeader title="Getting started" description="A short tour of the desk." />
      <ol className="space-y-6">
        {STEPS.map((s) => (
          <li key={s.n} className="flex gap-4">
            <span className="grid size-9 shrink-0 place-items-center rounded-full bg-brand text-xs font-medium text-bg">
              {s.n}
            </span>
            <div>
      ...
      <div className="rounded-xl bg-surface px-6 py-8 text-center shadow-[var(--kaze-shadow)]">
        <p className="font-display text-xl">Ready to work</p>
        <div className="mt-4">
          <Button
            asChild
            onClick={() => {
